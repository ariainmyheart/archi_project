...
int i_type(struct cpu_struct* cpu)
{
	...
	switch (ins->op) {
		case ADDI:
			...
			write_register(cpu, t, value, &status, ins->is_nop); //reg[t] = value
			break;
		case LW:
		...
		case SB:
			...
			break;
		case LUI:
			write_register(cpu, t, cu << 16, &status, ins->is_nop);
			break;
		case ANDI:
			write_register(cpu, t, s & cu, &status, ins->is_nop);
			break;
		case ORI:
			write_register(cpu, t, s | cu, &status, ins->is_nop);
			break;
		case NORI:
			...
			break;
		case SLTI:
			write_register(cpu, t, s < c, &status, ins->is_nop);
			break;
		case BEQ:
